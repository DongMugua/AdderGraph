...
mutable struct Hyperarc
    id::HyperarcID
    input_nodes::Vector{HypernodeID}
    output_node::HypernodeID
    weight::Int
end
...
function add_hyperarc!(h::Hypergraph,
    ...
    h.arcs_number += 1
    h.arc_id += 1
    hyperarc = Hyperarc(h.arcs_number, input_nodes, output_node, weight)
    for node_id in input_nodes
        push!(get!(h.hypernodes_arc_relation, node_id, Set{HyperarcID}()), h.arc_id)
    end
    push!(get!(h.hypernodes_arc_relation, output_node, Set{HyperarcID}()), h.arc_id)
...
function add_output_node_hyperarc!(h::Hypergraph, node_id::HypernodeID, arc_id::HyperarcID)
    push!(get!(h.hypernodes_arc_relation, node_id, Set{HyperarcID}()), arc_id)
    h.hyperarcs[arc_id].output_node = node_id
    return h
end
...
function remove_hypernode!(h::Hypergraph, node_id::HypernodeID)
    for arc_id in h.hypernodes_arc_relation[node_id]
        ...
        delete!(h.hyperarcs[arc_id].output_node, node_id)
        if isempty(h.hyperarcs[arc_id].input_nodes) || isempty(h.hyperarcs[arc_id].output_node)
            delete!(h.hyperarcs, arc_id)
            h.arcs_number -= 1
        end
    end
...
function get_arcs(h::Hypergraph, node_id::HypernodeID)
    return copy(h.hypernodes_arc_relation[node_id])
end

function get_input_nodes(h::Hypergraph, arc_id::HyperarcID)
    return copy(h.hyperarcs[arc_id].input_nodes)
end

function get_output_node(h::Hypergraph, arc_id::HyperarcID)
    return h.hyperarcs[arc_id].output_node
end
...
function get_possible_inputs(h::Hypergraph, node_id::HypernodeID)
    V = Vector{Vector{HypernodeID}}()
    for arc_id in get_arcs(h, node_id)
        input_nodes = get_input_nodes(h, arc_id)
        push!(V, input_nodes)
    end
...
function get_next_nodes(h::Hypergraph, node_id::HypernodeID)
    V = Set{HypernodeID}()
    for arc_id in get_arcs(h, node_id)
        output_node = get_output_node(h, arc_id)
        append!(V, output_node)
    end
...
function aset(u::Int, v::Int, w::Int, wordlength::Int)
    if u > v
        u, v = v, u
    end
    if u > w
    ...
    X = Set{Int}()
    union!(X, aset(u::Int, v::Int, w::Int, wordlength::Int))
    val_max = 1 << wordlength
    push!(X, odd(u+v))
    if u != v
        push!(X, odd(abs(u - v)))
...
function construct_hypertree(wordlength::Int, layers::Int = 3)
    ...
    add_hyperarc!(h, [node_id_root, node_id_root, 0], nodes_id_layer[current_layer][end], 0)
    for minusplusone in [-1,1]
        for w in 2:(wordlength-1)
            push!(nodes_id_layer[current_layer], add_node!(h, 2^w+minusplusone))
            add_hyperarc!(h, [node_id_root, node_id_root, 0], nodes_id_layer[current_layer][end], 1)
        end
    end
    ...
    if layers >= 2
        ...
        for node_id in nodes_id_layer[previous_layer]
            ...
            add_hyperarc!(h, [node_id, node_id, 0], new_node_id, 0)
        end
            ...
            for j in 2:length(nodes_id_layer[previous_layer])
                ...
                for value in X
                    if !(value in values_of_first_layer)
                        ...
                        add_hyperarc!(h, [id_i, id_j, 0], node_id, 1)
                    end
                end
            end
        end
